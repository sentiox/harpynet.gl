...
#     . /usr/lib/harpynet/sing_box_config_manager.sh
#
...
#   Writes updated JSON configuration to stdout
# Example:
#   CONFIG=$(sing_box_cm_configure_dns "$CONFIG" "direct-out" "ipv4_only" true true)
#######################################
sing_box_cm_configure_dns() {
    ...
    local reverse_mapping="$5"
    [ -n "$reverse_mapping" ] || reverse_mapping=false

    echo "$config" | jq \
        --arg final "$final" \
        --arg strategy "$strategy" \
        --argjson independent_cache "$independent_cache" \
        --argjson reverse_mapping "$reverse_mapping" \
        '.dns = {
			servers: (.dns.servers // []),
			rules: (.dns.rules // []),
			final: $final,
...
sing_box_cm_patch_dns_route_rule() {
    ...
    echo "$config" | jq \
        ...
        --arg key "$key" \
        --argjson value "$value" \
        'import "helpers" as h {"search": "/usr/lib/harpynet"};
        .dns.rules |= map(
...
#######################################
# Add an HTTP or HTTPS upstream proxy outbound.
# HTTPS means that the connection from the router to the proxy is protected
# with TLS; regular destination HTTPS still uses HTTP CONNECT in both modes.
#######################################
    ...
    echo "$config" | jq \
        ...
        '.outbounds += [(
            ...
            + (if $tls_enabled == "1" then {
                tls: ({enabled: true}
                    + (if $tls_server_name != "" then {server_name: $tls_server_name} else {} end))
              } else {} end)
        )]'
}
...
sing_box_cm_add_inline_ruleset_rule() {
    ...
    echo "$config" | jq -L /usr/lib/harpynet \
        --arg tag "$tag" \
        --arg key "$key" \
        --argjson value "$value" \
        'import "helpers" as h {"search": "/usr/lib/harpynet"};
